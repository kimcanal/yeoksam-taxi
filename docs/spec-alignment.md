...
- Frontend: `{ dong, date }` 요청, 응답 검증, 그래프/미니맵/히트맵 표시

서비스 handoff는 백엔드 수요 API 계약으로 받습니다. 백엔드가 동/날짜 기준 0-23시 수요 예측값을 제공하면, 프론트는 그 값을 그대로 그래프와 지도 선택 상태로 표시합니다.

### Module 4: Backend Demand API Handoff

현재 저장소는 운영 정책을 계산하지 않습니다. 백엔드가 동/날짜 기준으로 0-23시 수요 예측값을 내려주면, 프론트는 선택된 동의 시간대별 값을 그래프와 지도 패널에 표시하는 역할에 집중합니다.
...
- 미니맵 히트맵은 선택한 날짜와 0-23시 슬라이더 기준으로 동별 예측량을 색으로 보여줍니다.
