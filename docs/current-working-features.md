...
- **실시간 정밀 도로망**: 차선 구분선 및 도로의 물리적 고도 정보 표현
- **입체적 건물 구조**: 높이 정보에 맞춘 3D 빌딩 매스(Mass)와 상단의 반투명 지붕 레이아웃
- **행정동 영역 강조**: 동 단위의 바닥면 구분 및 실시간 동 경계 구획선 시각화
- **정밀 교통 인프라**: 신호 지점의 실시간 신호 제어기, 노면 정지선 및 입체 횡단보도 렌더링
- **다이내믹 에이전트**: 도로 네트워크 그래프를 타는 택시, 일반 차량 및 교차로 신호 연계 보행자의 실시간 주행/이동 애니메이션
...
- 수요 히트맵 시간대, 날씨 조회 시점 및 3D 그래픽스의 태양/달 물리 조명 씬이 실시간 시간축에 완벽히 동기화됩니다.
...
- **날씨 파티클 시스템**: 비(Rain), 눈(Snow) 강수 상태 시 화면 전체에 정밀 파티클 물리 물리계가 표현되며, 날씨 위젯에 날씨 심볼 배지가 동적으로 연동됩니다.
- **노면 반사(Sheen) 연동**: 비나 눈이 올 경우 도로의 금속성(Metalness)과 조도(Roughness) 계수를 실시간 제어하여 아스팔트가 물에 젖어 반짝이는 Specular Wet 표면 반사를 극적으로 연출합니다.
- **날씨별 속도 보정**: 폭우나 폭설 등 기상 악화 시 도로 위 에이전트(차량)들의 주행 속도 기본 계수가 자동으로 감속 보정되어 현실적인 주행 시뮬레이션을 구현합니다.
...
- **기본 일일 수요 데이터 요청**:
  ...
  ```
- **날짜 포맷 중계 변환**: 프론트엔드 프록시 레이어(`backend-proxy.ts`)에서 대시가 포함된 웹 표준 규격(`YYYY-MM-DD`)을 백엔드 DB 표준 규격인 `YYYYMMDD`로 매핑하여 호출 안전성을 보장합니다.
- **동별 24시간 상세 수요 요청**:
  ```text
...
수요 패널 상단의 시간 조작 모듈은 3D 가상 도시와 수치 대시보드의 실시간 시점을 정밀하게 일치시킵니다.
...
4. 해당 시각에 해당하는 9개 행정동의 호출 수요량이 실시간 재계산됩니다.
5. 미니맵 SVG 개별 행정동 색상 및 선택 동의 정밀 예측 수치가 고속 변환됩니다.
6. 3D 도시의 실시간 섀도 맵 각도, 조명 밝기 및 날씨 상태가 동일 프레임 타임라인에 안착합니다.
...
- 미니맵 패널의 고정밀 SVG 아웃라인 경계 구역에 색상 밀도(투명도 가중치가 반영된 주황/황금색 히트맵 그라디언트)를 부여합니다.
...
## 9. 실시간 수요 기반 에이전트(차량) 스케일링

백엔드 수요 API의 호출량 추이는 3D 지도를 달리는 가상 택시 에이전트의 총 개체 수에 즉각 유기적으로 매핑됩니다.

### 동작 원리
- 시간대별 시간당 수요 건수를 5분 단위의 디스플레이 슬롯 세부 수치로 균등 분할합니다.
- 모바일 및 웹 브라우저 렌더링 임계 한계를 방어하기 위해, 차량 개수의 최소/최대 가이드라인(`appliedTaxiCount`)과 스케일 백분율 상한 필터링 장치가 적용되어 있습니다.
- **독립적 에이전트 설계**: 지도를 횡단하는 전체 택시의 총 숫자는 특정 단일 동의 필터링에 영향받지 않으며, 전체 9개 동에 발생하는 실시간 총 택시 수요 합계를 기반으로 안전하게 렌더링되어 현실적인 강남 교통량 시뮬레이션을 보장합니다.
...
- **자가 치유형 예외 우회 파이프라인**: 프록시 단에서 백엔드 서버의 사설 SSL 만료 등으로 HTTPS 요청 실패가 감지되면, 세션을 중단하지 않고 즉시 비암호화 HTTP 프로토콜로 백업 우회 통신을 실행하여 데모 및 발표 현장에서 서비스가 끊기지 않도록 고가용성을 완벽히 조치했습니다.
...
### 현재 프론트엔드에 구현된 기능
- **공간 3D 환경**: OSM 강남/역삼권 9개 행정동의 고정밀 3D 입체 지도 가시화
- **그래픽스 피처**: 정밀 노면, 건물 3D 매스 및 반투명 지붕, 신호 체계, 대중교통 랜드마크 렌더링
- **다이내믹 에이전트**: 보행자 및 차량 에이전트의 도로 토폴로지 동적 시뮬레이션 주행
...
- **서버 프록시 연계**: 실시간 날씨 데이터 수집 및 백엔드 동별 수요 예측 수치 완벽 바인딩
- **사이드바 시각화**: 선택 동별 시간대 수요 곡선 및 9개 행정동 히트맵 연동 제어
- **스케일링**: 실시간 전체 수요 통계에 연동되어 3D 씬 내 택시 개수가 증감하는 동적 차량 생성 시스템

### 구현 범위에서 제외된 항목 (미구현 또는 백엔드 영역)
- **AI 수요 모델**: 딥러닝/머신러닝 기반의 수요량 자체를 학습하고 추정하는 연산 (백엔드 코어 영역)
- **실시간 차량 관제**: 실제 운행 중인 서울시 법인/개인 택시의 실시간 GPS 트래킹 연동 (시뮬레이션 가상 개체로 대체)
- **배차 알고리즘**: 매칭 최적화 및 배차 정책 시뮬레이션 연산
