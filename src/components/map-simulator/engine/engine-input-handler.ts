...
export function createEngineInputHandler(
  ...
  /** True while one or more touch fingers are on screen (used to pause overview auto-lerp). */
  const getIsTouchActive = () => activeTouchPointers.size > 0;
  ...
  const enterTouchMapMode = (): boolean => {
    ...
    // Mobile map style: overview supports free panning, so don't downgrade its
    // render budget. Only switch to drive when leaving follow/ride.
    if (
