...
export function createEngineCameraController(
  ...
  getIsTouchActive: () => boolean = () => false,
) {
  const {
    camera,
    cameraRig,
  ...
  const updateCameraForMode = (mode: CameraMode, delta: number) => {
    if (mode === "drive") {
      ...
      if (cameraFocusTargetRef.current) {
        const focusTarget = cameraFocusTargetRef.current;
        const targetDistance = THREE.MathUtils.clamp(focusTarget.distance, CAMERA_MIN_DISTANCE, maxMapDistance);
        ...
        if (
          ...
          Math.abs(cameraRig.pitch - focusTarget.pitch) < 0.02 &&
          Math.abs(cameraRig.distance - targetDistance) < 0.7 &&
          (focusTarget.yaw === undefined || Math.abs(wrapAngle(cameraRig.yaw - focusTarget.yaw)) < 0.03)
        ) {
          cameraFocusTargetRef.current = null;
        }
      }
