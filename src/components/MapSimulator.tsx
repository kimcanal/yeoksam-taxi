  ...
  return (
    <div className="relative h-full min-h-0 w-full overflow-hidden bg-[#060d16]">
      <section className="relative h-full overflow-hidden" style={layoutStyle}>
        ...
        {/* Ride exit button — mobile tap to leave taxi view */}
        {cameraMode === "ride" ? (
          <button
            type="button"
            aria-label="택시 뷰 나가기"
            onClick={() => setCameraMode(runtimeRefs.rideExitModeRef.current)}
            className="absolute right-4 top-4 z-30 flex h-11 w-11 items-center justify-center rounded-full border border-white/15 bg-slate-950/82 text-white shadow-2xl shadow-black/40 backdrop-blur-md active:scale-95"
          >
