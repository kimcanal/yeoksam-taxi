...
if (!/^\d{4}-\d{2}$/.test(startPeriod) || !/^\d{4}-\d{2}$/.test(endPeriod)) {
  console.error("Usage: node scripts/collect-seoul-living-population.mjs YYYY-MM:YYYY-MM");
  process.exit(1);
}

const keepRaw = process.argv.includes("--keep-raw") || process.env.KEEP_RAW_LIVING_POP === "1";
const rawDir = path.join(projectRoot, "data", "raw", "living_population");
const outDir = path.join(projectRoot, "data", "processed", "living_population");
...
async function fetchFileIndex() {
  const res = await fetch("https://data.seoul.go.kr/dataList/OA-14991/S/1/datasetView.do?tab=A");
  if (!res.ok) throw new Error(`OA-14991 file page failed: ${res.status}`);
  const html = await res.text();
  const files = new Map();
  const pattern =
    /title="(LOCAL_PEOPLE_DONG_(\d{6})\.zip)"\s+onclick="javascript:downloadFile\('([^']+)'\);?"/g;
...
async function downloadZip(period, meta) {
  ...
  const res = await fetch("https://datafile.seoul.go.kr/bigfile/iot/inf/nio_download.do?&useCache=false", {
    ...
    headers: {
      ...
      Referer: "https://data.seoul.go.kr/dataList/OA-14991/S/1/datasetView.do?tab=A",
    },
    body,
  });
...
async function processZip(period, zipPath) {
  const processor = path.join(projectRoot, "scripts", "process_seoul_living_population_zip.py");
...
const statusPath = path.join(outDir, `seoul_living_population_collection_${startPeriod}_${endPeriod}.status.json`);
